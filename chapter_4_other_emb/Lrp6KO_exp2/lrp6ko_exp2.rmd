...
test.mod <- model.matrix(~ as.factor(Group), data = ko1.neg.data.pheno)
set.seed(2018)
ko1.neg.sv.test <- sva(ko1.neg.data.for.sva, test.mod, ko1.neg.mod0)

test2 <- ko1.neg.sv.test$sv
colnames(test2) <- c("test_S1", "test_S2")
test3 <- ko1.neg.sv$sv
colnames(test3) <- c("orig_S1", "orig_S2", "orig_S3")
test4 <- test2 %>% 
  as_tibble() %>% 
  bind_cols(
    test3 %>% 
...
ko1.neg.design.pca <- cbind(ko1.neg.design, test4$PC1)
colnames(ko1.neg.design.pca) <- c(
  "A_ko_10ppmFA", "N_wt_10ppmFA", 
  "A_ko_2ppmFA", "N_wt_2ppmFA", 
  "PC1"
  )
ko1.neg.pc1.fit <- lmFit(ko1.neg.data.for.sva, ko1.neg.design.pca)

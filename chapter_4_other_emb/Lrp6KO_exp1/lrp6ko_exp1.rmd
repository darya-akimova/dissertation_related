...
ko1.pos.noNA.gathered <- ko1.pos.noNA %>% 
  gather(
    key = "Metabolite", "Abundance", 
    which(colnames(ko1.pos.noNA) == "Lrp6KOp1"):ncol(ko1.pos.noNA)
    )

ko1.pos.noNA.gathered %>% 
  ggplot(aes(Samples, Abundance, fill = Type)) +
  geom_boxplot() +
  geom_boxplot(aes(color = Type), fatten = NULL, fill = NA, coef = 0, outlier.alpha = 0, show.legend = FALSE) +
  theme(axis.text.x = element_text(angle = 90))
...
ko1.pos.noNA.gathered %>% 
  filter(Folate == "10ppm" & Status == "A ko/ko") %>% 
  ggplot(aes(Samples, Abundance, fill = Type)) +
  geom_boxplot() +
  geom_boxplot(aes(color = Type), fatten = NULL, fill = NA, coef = 0, outlier.alpha = 0, show.legend = FALSE) +
  theme(axis.text.x = element_text(angle = 90))
...
ko1.pos.smpl.pca.x %>% 
  unite("Treatment", Folate, Status, sep = " / ") %>% 
  ggplot(aes(x = PC1, y = PC2, color = Treatment, label = rownames(ko1.pos.smpl.pca.x))) +
  geom_point(size = 4, alpha = 0.8) +
  geom_text(hjust = 0)
# remove "emb_37_6" from the pos mode dataset
ko1.pos.smpl.filt.pca <- ko1.pos.noNA %>% 
  filter(Type == "sample" & Samples != "emb_37_6") %>% 
  select(starts_with("Lrp6KOp")) %>% 
  as.matrix() %>% 
  prcomp()
ko1.pos.smpl.filt.pca.x <- as.data.frame(ko1.pos.smpl.filt.pca$x)
ko1.pos.smpl.filt.pca.x <- ko1.pos.smpl.filt.pca.x %>% 
  bind_cols(
    ko1.pos.noNA %>% 
      filter(Type == "sample" & Samples != "emb_37_6") %>% 
      select(Samples, Folate:Status)
  )
row.names(ko1.pos.smpl.filt.pca.x) <- ko1.pos.smpl.filt.pca.x$Samples
ko1.pos.smpl.filt.pca.x %>% 
  unite("Treatment", Folate, Status, sep = " / ") %>% 
  ggplot(aes(x = PC1, y = PC2, color = Treatment)) +
  geom_point(size = 4, alpha = 0.8)
...
# sva calculation
ko1.neg.mod <- model.matrix(~ as.factor(Folate) + as.factor(Status), data = ko1.neg.data.pheno)
ko1.neg.mod0 <- model.matrix(~ 1, data = ko1.neg.data.pheno)
set.seed(2018)
num.sv(ko1.neg.data.for.sva, ko1.neg.mod, method = "be")
set.seed(2018)
num.sv(ko1.neg.data.for.sva, ko1.neg.mod, method = "leek")
set.seed(2018)
ko1.neg.sv <- sva(ko1.neg.data.for.sva, ko1.neg.mod, ko1.neg.mod0)
...
ko1.neg.design.sv <- cbind(ko1.neg.design, ko1.neg.sv$sv[, 1])
colnames(ko1.neg.design.sv) <- c("A_ko_10ppmFA", "N_wt_10ppmFA", "A_ko_2ppmFA", "N_wt_2ppmFA", "S1")
...
colnames(ko1.neg.design) <- c("A_ko_10ppmFA", "N_wt_10ppmFA", "A_ko_2ppmFA", "N_wt_2ppmFA")
ko1.neg.fit.nosv <- lmFit(ko1.neg.data.for.sva, ko1.neg.design)
ko1.neg.cont.mat.nosv <- makeContrasts(
  ko_lowFA = A_ko_2ppmFA - N_wt_2ppmFA, 
  ko_highFA = A_ko_10ppmFA - N_wt_10ppmFA,
  FA_diff = (A_ko_10ppmFA - N_wt_10ppmFA) - (A_ko_2ppmFA - N_wt_2ppmFA),
  levels = c(
...
ko1.neg.nosv.result <- contrasts.fit(ko1.neg.fit, ko1.neg.cont.mat) %>% 
  eBayes() %>% 
  decideTests(method = "global", adjust.method = "BH")
vennDiagram(ko1.neg.nosv.result)
```
...
ko1.pos.design.sv <- cbind(ko1.pos.design, ko1.pos.sv$sv[, 1])

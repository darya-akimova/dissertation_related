...
```{r}
all.niac.metab <- bind_rows(
  bind_rows(
    "vpa_only" = vpa.pathways %>% 
      filter(str_detect(path_name, "Nicotinate and nicotinamide metabolism") | str_detect(path_name, "Tryptophan metabolism") | str_detect(path_name, "Vitamin B6 metabolism")),
...
length(unique(all.niac.metab$compound_full_exp))
```
...
vpa.hits.df <- bind_rows(
  ...
  filter(compound_short %in% all.niac.metab$compound_short)
### vpa + fa exp 1 ###
...
### vpa only ###
vpa.niac.abun.df <- vpa.abun.list %>% 
  # pick only relevant sample info and compound abun columns (nucleotide metab)
  map(select, Samples, Treatment, one_of(all.niac.metab$compound_short)) %>% 
  ...
  map(inner_join, all.niac.metab, by = "compound_short") %>% 
  # list -> df
  bind_rows() %>%
  # rearrange the columns
  select(experiment, mode, Samples:compound_short, compound_full_exp, everything())
### vpa + fa exp 1 ###
vf.niac.abun.df <- vf.abun.list %>%  
  map(select, Samples:FA, one_of(all.niac.metab$compound_short)) %>% 
  ...
  select(experiment, mode, Samples:compound_short, compound_full_exp, everything())
### vpa hilic ###
vpa.hilic.niac.abun.df <- vpa.hilic.abun.list %>% 
  map(select, Samples, Group, one_of(all.niac.metab$compound_short)) %>% 
  ...
  select(experiment, mode, Samples:compound_short, compound_full_exp, everything())
### vpa + fa p5 hilic ###
vf.p5.niac.abun.df <- vf.p5.abun.list %>%  
  map(select, Samples:FA, one_of(all.niac.metab$compound_short)) %>% 
  map(gather, key = "compound_short", value = "log2_resid_abun", -c(Samples:FA)) %>% 
  map(inner_join, all.niac.metab, by = "compound_short") %>% 
  bind_rows() %>% 
...
### vpa only ###
vpa.niac.abun.df <- vpa.niac.abun.df %>% 
...
### vpa + fa exp 1 ###
vf.niac.abun.df <- vf.niac.abun.df %>% 
...
### vpa + fa p5 hilic ###
vf.p5.niac.abun.df <- vf.p5.niac.abun.df %>% 
...
vpa.niac.abun.df %>% 
  ...
  ggtitle("Nicotinate and nicotinamide metabolism (All detecteded)\nVPA-only (Exp 1)") +
  xlab("Compound Name") +
  ylab("log2(Compound Residuals)") +
  facet_wrap(~ significant, ncol = 1)
vpa.niac.abun.df %>% 
  filter(path_name == "Vitamin B6 metabolism") %>% 
  mutate(alt_cmpnd_name = paste(compound_full_exp, " (", mode, ")", sep = "")) %>% 
  ggplot(aes(alt_cmpnd_name, log2_resid_abun, color = Treatment, fill = Treatment)) +
  geom_boxplot(alpha = 0.5) +
  geom_point(size = 2, position = position_dodge(0.75)) +
  ...
  ggtitle("Vitamin B6 metabolism (All detected)\nVPA-only (Exp 1)") +
  xlab("Compound Name") +
  ylab("log2(Compound Residuals)") +
  facet_wrap(~ significant, ncol = 1)
vpa.niac.abun.df %>% 
  filter(path_name == "Tryptophan metabolism") %>% 
  mutate(alt_cmpnd_name = paste(compound_full_exp, " (", mode, ")", sep = "")) %>% 
  ggplot(aes(alt_cmpnd_name, log2_resid_abun, color = Treatment, fill = Treatment)) +
  geom_boxplot(alpha = 0.5) +
  geom_point(size = 2, position = position_dodge(0.75)) +
  ...
  ggtitle("Tryptophan metabolism (All detected)\nVPA-only (Exp 1)") +
  xlab("Compound Name") +
  ylab("log2(Compound Residuals)") +
  facet_wrap(~ significant, ncol = 1)
...
vf.niac.abun.df %>% 
  ...
  mutate(alt_cmpnd_name = paste(compound_full_exp, " (", mode, ")", sep = "")) %>% 
  ggplot(aes(alt_cmpnd_name, log2_resid_abun, color = VPA, fill = VPA)) +
  geom_boxplot(alpha = 0.5) +
  geom_point(size = 2, position = position_dodge(0.75)) +
  scale_color_manual(values = c("#56B4E9", "#E69F00"), labels = c("Control", "VPA")) +
  scale_fill_manual(values = c("#56B4E9", "#E69F00")) +
  ...
  ggtitle("Nicotinate and nicotinamide metabolism (All detected)\nVPA+FA Exp (Exp 3)") +
  xlab("Compound Name") +
  ylab("log2(Compound Residuals)") +
  facet_grid(FA ~ significant, scales = "free_x")
vf.niac.abun.df %>% 
  filter(path_name == "Vitamin B6 metabolism") %>% 
  separate(Treatment, into = c("VPA", "FA"), sep = "_") %>% 
  mutate(alt_cmpnd_name = paste(compound_full_exp, " (", mode, ")", sep = "")) %>% 

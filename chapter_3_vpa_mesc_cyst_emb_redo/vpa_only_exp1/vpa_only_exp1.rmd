...
overview.fig.ppt.p1 <- plot_grid(
    vpa.full.mass.v.rt.plot, 
    vpa.cell.neg.smpl.miss, 
    ncol = 2
    )

overview.fig.ppt.p2 <- plot_grid(
  vpa.cell.neg.prof.plot,
  plot_grid(
    vpa.cell.neg.solv.v.empty.plot, vpa.cell.neg.treat.dist.plot, ncol = 2
    ),
  ncol = 1
  )
#save_plot("./results/vpa_only_exp1_overview_fig_pptp1.png", overview.fig.ppt.p1, base_width = 6, base_height = 3)
save_plot("./results/vpa_only_exp1_overview_fig_pptp2.png", overview.fig.ppt.p2, base_width = 6, base_height = 6)
```
...
#save_plot("./results/vpa_metab_plot.png", vpa.metabolism.grid.plot, base_height = 6, base_width = 8)
```
...
vpa.cell.neg.prot.conc.facet <- vpa.cell.neg.covar %>% 
  ...
  theme_minimal() +
  scale_color_stata(guide = FALSE) +
  xlab("Sample Protein Concentration") +
  ylab("Variable Value")
...
vpa.cell.neg.modSV.resid.heatmap <- vpa.cell.neg.resid.for.profile %>% 
  ...
  mutate(scaled_abun = cut(scaled_abun, breaks = seq(-4, 4, by = 1), right = FALSE)) %>% 
  mutate(Treatment = case_when(Treatment == "cntrl" ~ "Control", Treatment == "vpa" ~ "VPA", TRUE ~ "Don't know")) %>% 
...
#save_plot("./results/vpa_cell_neg_heatmap_grid_notscaled.png", vpa.cell.neg.heatmap.grid, base_height = 10, base_width = 7)

vpa.cell.neg.modSV.resid.heatmap.continuous <- vpa.cell.neg.resid.for.profile %>% 
  gather("Compound", "scaled_abun", -c(Samples:Treatment)) %>% 
  mutate(compound_order = factor(Compound, levels = vpa.cell.neg.resid.order$compound_sort)) %>% 
  mutate(sample_order = factor(Samples, levels = vpa.cell.neg.modSV.resid$Samples[vpa.cell.neg.resid.clust$order])) %>% 
  mutate(Treatment = case_when(Treatment == "cntrl" ~ "Control", Treatment == "vpa" ~ "VPA", TRUE ~ "Don't know")) %>% 
  ...
  facet_wrap(~ Treatment, ncol = 1, scales = "free")
vpa.cell.neg.heatmap.grid.cont <- plot_grid(
  vpa.cell.neg.modSV.resid.prof.plot,
  vpa.cell.neg.modSV.resid.heatmap.continuous,
  ncol = 1,
  labels = c("A", "B"),
  rel_heights = c(1, 1.5)
)
vpa.cell.neg.heatmap.grid.cont
#save_plot("./results/vpa_cell_neg_heatmap_grid_notscaled_cont.png", vpa.cell.neg.heatmap.grid.cont, base_height = 10, base_width = 7)
...
vpa.cell.pos.resid.for.profile <- vpa.cell.pos.modSV.resid %>% 
  select(Samples:Treatment, one_of(vpa.cell.pos.top.w.info$compound_short)) 
...
vpa.cell.pos.modSV.resid.prof.plot <- vpa.cell.pos.resid.for.profile %>% 
  ...
  scale_color_manual(values = c("#56B4E9","#E69F00"), labels = c("Control", "VPA")) +
  ylab("log2(Compound Resid)") +
  ggtitle("Positive MS Mode Compound Residuals") +
  # overlay the group averages 
  geom_line(data = vpa.cell.pos.resid.order %>% gather("Treatment", "treat_mean", cntrl:vpa), aes(compound_sort, treat_mean, color = Treatment, group = Treatment),
    size = 2
    )
vpa.cell.pos.modSV.resid.prof.plot
vpa.cell.pos.modSV.resid.prof.plot.alt <- vpa.cell.pos.resid.for.profile %>% 
  gather("Compound", value = "Abundance", -c(Samples, Treatment)) %>% 
  mutate(compound_order = factor(Compound, levels = vpa.cell.pos.resid.order$compound_sort)) %>% 
  ggplot(aes(compound_order, Abundance, color = Treatment, group = Samples)) + 
  geom_line(alpha = 0.3, size = 1.25) + 
  ...
  coord_flip()
vpa.cell.pos.modSV.resid.prof.plot.alt 
...
vpa.cell.neg.modSV.resid.prof.plot <- vpa.cell.neg.resid.for.profile %>% 
  ...
  scale_color_manual(values = c("#56B4E9","#E69F00"), labels = c("Control", "VPA")) +
  ylab("log2(Compound Resid)") +
  ggtitle("Negative MS Mode Compound Residuals") +
  # overlay the group averages 
  geom_line(data = vpa.cell.neg.resid.order %>% gather("Treatment", "treat_mean", cntrl:vpa), aes(compound_sort, treat_mean, color = Treatment, group = Treatment),
    size = 2
    )
...
vpa.cell.neg.modSV.resid.prof.plot.alt <- vpa.cell.neg.resid.for.profile %>% 
  gather("Compound", value = "Abundance", -c(Samples, Treatment)) %>% 
  mutate(compound_order = factor(Compound, levels = vpa.cell.neg.resid.order$compound_sort)) %>% 
  ggplot(aes(compound_order, Abundance, color = Treatment, group = Samples)) + 
  geom_line(alpha = 0.3, size = 1.25) + 
  ...
  coord_flip()
vpa.cell.neg.modSV.resid.prof.plot.alt
#save_plot("./results/vpa_cell_pos_profile.png", vpa.cell.pos.modSV.resid.prof.plot, base_height = 6, base_width = 8)
#save_plot("./results/vpa_cell_neg_profile.png", vpa.cell.neg.modSV.resid.prof.plot, base_height = 6, base_width = 8)


#save_plot("./results/vpa_cell_pos_profile_flip.png", vpa.cell.pos.modSV.resid.prof.plot.alt, base_height = 6, base_width = 5)
#save_plot("./results/vpa_cell_neg_profile_flip.png", vpa.cell.neg.modSV.resid.prof.plot.alt, base_height = 6, base_width = 6)
```

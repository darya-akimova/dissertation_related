...
### stat tests
ser.neg.test <- ser.neg.per %>%
  select(-c(RT:Ser)) %>% 
  gather("isotope", "per_abun", m_0_per:m_3_per) %>% 
  group_by(isotope) %>% 
  nest() %>% 
  mutate(model = map(data, ~lm(per_abun ~ Treatment, data = .))) %>% 
  mutate(tidy_model = map(model, tidy)) %>% 
  unnest(tidy_model) %>% 
  filter(term != "(Intercept)") %>% 
  mutate(which_test = "ser_neg")
ser.pos.test <- ser.pos.per %>%
  select(-c(RT:Ser)) %>% 
  gather("isotope", "per_abun", m_0_per:m_3_per) %>% 
  group_by(isotope) %>% 
  nest() %>% 
  ...
  unnest(tidy_model) %>% 
  filter(term != "(Intercept)") %>% 
  mutate(which_test = "ser_pos")
amp.neg.test <- amp.neg.per %>%
  select(-c(RT:Ser)) %>% 
  gather("isotope", "per_abun", m_0_per:m_2_per) %>% 
  group_by(isotope) %>% 
  nest() %>% 
  mutate(model = map(data, ~lm(per_abun ~ Treatment, data = .))) %>% 
  mutate(tidy_model = map(model, tidy)) %>% 
  unnest(tidy_model) %>% 
  filter(term != "(Intercept)") %>% 
  mutate(which_test = "amp_neg")
atp.neg.test <- atp.neg.per %>%
  select(-c(RT:Ser)) %>% 
  gather("isotope", "per_abun", m_0_per:m_4_per) %>% 
  group_by(isotope) %>% 
  nest() %>% 
  mutate(model = map(data, ~lm(per_abun ~ Treatment, data = .))) %>% 
  mutate(tidy_model = map(model, tidy)) %>% 
  unnest(tidy_model) %>% 
  filter(term != "(Intercept)") %>% 
  mutate(which_test = "atp_neg")
dttp.neg.test <- dttp.neg.per %>%
  select(-c(RT:Ser)) %>% 
  gather("isotope", "per_abun", m_0_per:m_1_per) %>% 
  group_by(isotope) %>% 
  nest() %>% 
  mutate(model = map(data, ~lm(per_abun ~ Treatment, data = .))) %>% 
  mutate(tidy_model = map(model, tidy)) %>% 
  unnest(tidy_model) %>% 
  filter(term != "(Intercept)") %>% 
  mutate(which_test = "dttp_neg")
iso_stat_tests <- ser.neg.test %>% 
  bind_rows(ser.pos.test) %>% 
  bind_rows(amp.neg.test) %>% 
  bind_rows(atp.neg.test) %>% 
  bind_rows(dttp.neg.test) %>% 
  mutate(adj_pval = p.adjust(p.value, method = "bonferroni"))
iso_stat_tests
iso_stat_tests %>% 
  filter(adj_pval < 0.05)
```
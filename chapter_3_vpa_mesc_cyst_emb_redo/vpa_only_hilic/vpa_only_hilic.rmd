...
vpa.cell.neg.resid.order <- vpa.cell.neg.resid.for.profile %>% 
  ...
  mutate(
    compound_full_alt = case_when(
      compound_full == "Docosahexaenoic Acid (22:6 n-3)" ~ "Docosahexaenoic\nAcid",
      compound_full == "N-Acetylaspartyl Glutamic Acid" ~ "N-Acetylaspartyl-\nglutamate",
      TRUE ~ compound_full
      )
  )
vpa.cell.neg.modSV.resid.prof.plot <- vpa.cell.neg.resid.for.profile %>% 
  ...
  scale_x_discrete(name = "Compound", labels = vpa.cell.neg.resid.order$compound_full_alt) +
...
vpa.cell.pos.resid.order <- vpa.cell.pos.resid.for.profile %>% 
  ...
  mutate(
    compound_full_alt = case_when(
      compound_full == "Glycerol-3-phosphocholine" ~ "Glycerol-\n3-phosphocholine",
      compound_full == "Nicotinamide Mononucleotide" ~ "NMN",
      compound_full == "N-Acetylaspartyl Glutamic Acid" ~ "N-Acetylaspartyl-\nglutamate",
      TRUE ~ compound_full
      )
    )
vpa.cell.pos.modSV.resid.prof.plot <- vpa.cell.pos.resid.for.profile %>% 
  ...
  scale_x_discrete(name = "Compound", labels = vpa.cell.pos.resid.order$compound_full_alt) +
  scale_color_manual(values = c("#56B4E9","#E69F00"), labels = c("Control", "VPA")) +
  ylab("log2(Compound Resid)") +
  # overlay the group averages 
  geom_line(
...
#save_plot(filename = "./results/vpa_hilic_sig_resid_plots_smaller.png", vpa.hilic.resid.grid, base_width = 7, base_height = 6)
```

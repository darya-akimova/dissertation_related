...
dependency.plot <- depen %>% 
  mutate(Fuel = ifelse(str_detect(Fuel, "fatty"), "fatty acid", Fuel)) %>% 
  ggplot(aes(Treatment, dependency, color = Treatment)) +
  geom_boxplot() +
...
capacity.plot <- capacity %>% 
  mutate(Fuel = ifelse(str_detect(Fuel, "fatty"), "fatty acid", Fuel)) %>% 
  ggplot(aes(Treatment, capacity, color = Treatment)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_tableau() +
  facet_wrap(~ Fuel) +
...
flex.plot <- flex %>% 
  ...
  mutate(
    ...
    Fuel = ifelse(str_detect(Fuel, "fatty"), "fatty acid", Fuel)
    ) %>% 
